...
const Education = () => {
    const educationHistory = [
        {
            ...
            duration: '2023 - Present (Second Year)', // Updated to reflect "Second Year"
    ...
    return (
        // FINAL COLOR FIX: Light Mode (bg-surface/Cream) | Dark Mode (dark:bg-background/Black)
            ...
            <div className="container mx-auto px-4 max-w-4xl">
                ...
                <div className="space-y-12">
                    {educationHistory.map((item, index) => (
                        <div key={index} className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-8">
  ...
  <h3 className="text-xl font-bold text-primary">
    {item.institution}
                                ...
                                {/* Duration text swaps correctly */}
                                <p className="text-md italic text-background/70 dark:text-surface/70">
                                    {item.duration}
                                </p>
                            </div>
                            ...
                            <div className="md:w-2/3 border-l-4 border-secondary/50 pl-6">
                                {/* Degree title text color swaps correctly, primary is still an accent color */}
                                <h4 className={`text-2xl font-semibold mb-2 text-secondary`}>
                                    {item.degree}
                                </h4>
                                {/* Details text color swaps correctly */}
                                <p className="text-lg text-background/80 dark:text-surface/80">
                                    {item.details}
                                </p>
                                {/* Added a focus badge for your current primary education */}
                                {item.primary && (
